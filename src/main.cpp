...
void loop() 
{
    ...
    uint8_t sensorValue = tracker.readSensors();
    tracker.printSensorStates();

    if (nfc.readCard()) {
        Serial.println("检测到NFC卡");
        Serial.print("类型: ");
        Serial.println(nfc.getCardType());
        Serial.print("UID: ");
    ...
    switch(sensorValue) {
        ...
        motorController.setSpeedOpenLoop(0, 0);
            break;
        case 0b0110: // 正常前进状态（中间两个传感器检测到黑线）
        motorController.setSpeedOpenLoop(80, 80);
            break;
        case 0b0010: // 轻微右偏
        motorController.setSpeedOpenLoop(80, 100);
            break;
        case 0b0100: // 轻微左偏
        motorController.setSpeedOpenLoop(100, 80);
            break;
        case 0b1111: // 检测到十字路口
        motorController.setSpeedOpenLoop(0, 0);
            break;
